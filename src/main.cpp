...
int main() {
    ...
    solver.setTemperature(298.0);  // 25°C
    double energy_25c = solver.computeBindingEnergy(phage, bacteria);
    std::cout << "   Temperature 25°C: Debye length = " << std::fixed << std::setprecision(3)
              << solver.getDebyeLength() << " nm, Energy = " 
              << std::fixed << std::setprecision(2) << energy_25c << " kJ/mol" << std::endl;
    
    // Back to physiological
    solver.setTemperature(310.0);
    std::cout << std::endl;
    
    // 5. Test with T4 Phage (if available)
